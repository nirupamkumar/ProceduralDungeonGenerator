...
public class TileSpawner : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int x = -1; x <= 1; x++)
        {
            for (int y = -1; y <= 1; y++)
            {
                Vector2 tarPos = new Vector2(transform.position.x + x, transform.position.y + y);
                Collider2D hit = Physics2D.OverlapBox(tarPos, hitSize, 0, envMask);

                if (!hit)
                {
                    // Add a wall
                    GameObject goWall = Instantiate(dungeonManager.wallPrefab, tarPos, Quaternion.identity) as GameObject;
